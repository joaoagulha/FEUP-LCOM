...
int timer_set_frequency(unsigned char timer, unsigned long freq) {
	...
	unsigned char previous_timer_configuration, read_back_command,
			timer_selector, LSB, MSB;
	...
	previous_timer_configuration = 0;

	//returns Timer 0 configuration
	if (timer_get_conf(timer, &previous_timer_configuration) != FINE) {
		printf(
	...
	read_back_command = (previous_timer_configuration & BIT(0)) | TIMER_SQR_WAVE
			| TIMER_LSB_MSB;
...
void timed_scan_int_handler(unsigned int *time) {
	(*time)++;
}

int timer_get_conf(unsigned char timer, unsigned char *st) {

	char readBackCommand;
	...
	//2 MSBs =1
	readBackCommand = TIMER_RB_SEL(timer) | TIMER_RB_COUNT_ | TIMER_RB_CMD; // command for Control Register
	...
	//							 0x41 -> Timer1
	//					 		 0x42 -> Timer2
	unsigned char timer_selector = TIMER_0 + timer;
...
int timer_test_int(unsigned long time) {
	...
	if ((irq_set = timer_subscribe_int()) == -NOT_FINE) { //subscribe timer0
		printf("\n->timer_test_int() -> Error subscribing");
		return NOT_FINE;
	}

	interrupt_counter = 0;
	int ipc_configuration = 0;
